var Theme = {
    ...
    miscScripts: function($){
        if($('body.logged-in').length > 0){
            $('#menu-item-110 a').text('My Account');
    ...
    configureScripts: function($){
        if($('#package-tabs').length > 0){
            ...
            $('#package-tabs a').click(function(){
                ...
                setTimeout(function(){Theme.checkCalcPrice($);}, 100);
            });

            //no of articles
            $('div[data-name=no_of_articles] input[type=range]').change(function(){
    ...
    checkCalcPrice: function($) {
        var nowords = parseInt($('div[data-name=words_per_article] input[type=number]').val());
        var noa = parseInt($('div[data-name=no_of_articles] input[type=number]').val());
        ...
        console.log('noa', noa);
        var p = parseFloat($('#package-tabs a.active').attr('product-price'));
    ...
    fontaweDetails: function($){
        $('.details ul li').prepend('<i class="fa-solid fa-check"></i>');
    },

    customTab: function($){
        if($('.custom-tab').length > 0){
            $('.custom-tab__navs-item').click(function(){
                const n = $(this).attr('nav-id');

                $('.custom-tab__item').removeClass('active');
                $('.custom-tab__navs-item').removeClass('active');
                $('.custom-tab__item[tab-id="'+n+'"]').addClass('active');
